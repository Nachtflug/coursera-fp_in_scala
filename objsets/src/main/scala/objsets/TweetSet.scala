...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = reweetedLoop(elem)
...
object GoogleVsApple {
  ...
  def keywordFilter(all: TweetSet, keyword: List[String]) =
    all.filter(_.text.split(" ").exists(keyword.contains(_)))

  lazy val googleTweets: TweetSet = keywordFilter(TweetReader.allTweets, google)

  lazy val appleTweets: TweetSet = keywordFilter(TweetReader.allTweets, apple)
