...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.groupBy(_.toLower).map {
      tuple => (tuple._1, tuple._2.length)
    }.toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    occurrences.foldLeft(List[Occurrences](List())){
      case (l, (c, n)) => { for(i <- l; m <- 1 to n) yield ((c, m) :: i).sorted } ::: l
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val withZero = for (ox <- x) yield
      (ox._1, ox._2 - y.toMap.getOrElse(ox._1, 0))
    withZero.filter(_._2 > 0)
  }
    ...
    def sentenceAnagrams(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty)
        ...
        for {
          ...
          word <- dictionaryByOccurrences.getOrElse(occurencesCombination, Nil)
